...
```{r, include=FALSE}
save_plot("Figures/Figure5.pdf", grid.grabExpr(draw(f5)), base_width = 6.69, base_height = 6)
```
...
fs7_1 <- plot_grid(p1, p2, rel_widths = c(1, 4))
fs7_1
```
<br>
<br>

#### 2.3 Method with the best informative covariate
```{r, message=FALSE, warning=FALSE}
fs7_2 <- left_join(discovery_number, reference_discovery, by = "dataset") %>%
  mutate(logFC = log2((discovery + 1)/(ST + 1))) %>%
  group_by(dataset, method) %>%
...
<br>
```{r}
fs7 <- plot_grid(fs7_1, fs7_2, labels = c("A", "B"), ncol = 1, label_size = 18)
fs7
```

```{r, include=FALSE}
save_plot("Figures/FigureS7.pdf", fs7, base_width = 6.69, base_height = 4)
```
...
fs8 <- ggplot(data = EWAS33_data, mapping = aes(x = percentile_covariate, y = -log10(p_value))) +
  geom_hex(bins = 100) + 
...
```{r, include=FALSE}
save_plot("Figures/FigureS8.pdf", plot = fs8, base_width = 7, base_height = 2.5)
```
...
fs9 <- plot_grid(p1, p2, rel_widths = c(1, 4))
fs9
```
