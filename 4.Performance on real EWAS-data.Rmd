...
# heatmap body annotation
annotation_df <- dcast(omnibus_result, dataset ~ measure, value.var = "p.value")
rownames(annotation_df) <- annotation_df$dataset
annotation_df$dataset <- NULL
annotation_df <- annotation_df[rownames(wide_data), colnames(wide_data)[1:12]]
annotation_df$most.sig <- 1
annotation_df$multiple <- 1
annotation_matrix <- as.matrix(annotation_df)
...
fs12 <- Heatmap(as.matrix(wide_data[, 1:14]),
        ...
        cell_fun = function(j, i, x, y, width, height, fill) {
          if(!is.na(annotation_matrix[i, j])){
            if(annotation_matrix[i, j]  < 0.05)
              grid.text("*", x, y, vjust = 0.8, gp = gpar(fontsize = 4))
          }},
        top_annotation = columnAnnotation(
