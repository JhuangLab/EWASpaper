...
fs11 <- ggplot(data = cdata, aes(x = factor(sample_size), y = mean, colour = measure)) +
...
```{r, include=FALSE}
ggsave("Figures/FigureS11.pdf", fs11, width = 7, height = 4, useDingbats = FALSE)
```
...
fs12 <- as.data.frame(ranks[selected_method])[selected_cpg, ] %>%
  melt(value.name = "Significance_rank") %>%
...
```{r, include=FALSE}
save_plot("Figures/FigureS12.pdf", fs12, base_width = 6.69, base_height = 5)
```
...
draw(fs13_1)
fs13_1 <- grid.grabExpr(draw(fs13_1))
...
fs13_2 <- ranks[[selected_method]][selected_cpg, ] %>%
  select(selected_measure) %>%
  melt(value.name = "Significance_rank") %>%
  group_by(variable) %>%
  mutate(significance_rank_median = median(Significance_rank)) %>%
...
selected_method <- c("CAMT", "AdaPT", "IHW", "BL", "FDRreg")
fs13_3 <- subset_rank[selected_cpg, ] %>%
  select(selected_method) %>%
  melt(value.name = "Significance_rank") %>%
  ggplot(aes(x = variable, y = Significance_rank)) +
  geom_boxplot(aes(fill = variable), width = 0.72, fatten = 1) +
...
bottom_row <- plot_grid(fs13_2, fs13_3, labels = c("B", "C"), align = "h", nrow = 1, label_size = 18)
fs13 <- plot_grid(fs13_1, bottom_row, labels = c("A", ""), ncol = 1, label_size = 18)
fs13
```

```{r, include=FALSE}
save_plot("Figures/FigureS13.pdf", plot = fs13, base_width = 6, base_height = 6)
```
...
```{r, include=FALSE}
save_plot("Figures/FigureS14.pdf", plot = fs14, base_width = 5, base_height = 6)
```

#### 3.3 Intersetion of method IHW and CAMT
```{r venn plot, fig.width=4, fig.height=3}
fs15 <- draw.pairwise.venn(area1 = length(IHW),
                           area2 = length(CAMT),
...
```{r, include=FALSE}
save_plot("Figures/FigureS15.pdf", fs15)
```
