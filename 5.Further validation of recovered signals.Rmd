...
### 1. Down-sampling analysis {#downsample}
***
EWAS51 is a dataset with binary phenotype (FASD and control). We sampled each phenotype randomly at sample size 10, 20, 30, 40, 50, 60, 70, 80, 90 and 100 by method IHW and CAMT. This document displays the power of each method with different covariates.  
<br>
<br>
...
#   ggplot(aes(x = factor(sample_size), y = Power, color = measure)) + 
#   stat_summary(fun.y = mean, geom = "point", position = position_dodge(width = 0.5), na.rm = TRUE, lwd = 0.15) +
#   stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.8, position = position_dodge(width = 0.5), na.rm = TRUE, lwd = 0.15) +
...
cdata <- ddply(IHW_power, c("measure", "sample_size"), summarise,
               N    = sum(!is.na(Power)),
...
#   filter(measure %in% selected_measure, sample_size %in% selected_samplesize) %>%
#   ggplot(aes(x = factor(sample_size), y = Power, color = measure)) + 
#   stat_summary(fun.y = mean, geom = "point", position = position_dodge(width = 0.5), na.rm = TRUE) +
#   stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.8, position = position_dodge(width = 0.5), na.rm = TRUE) +
#   scale_color_manual(values = measure_col) +
#   theme_bw() +
#   xlab("sample size") +
#   theme(legend.key.size = unit(0.5, "cm"),
...
cdata <- ddply(CAMT_power, c("measure", "sample_size"), summarise,
               N    = sum(!is.na(Power)),
               mean = mean(Power, na.rm = TRUE),
               sd   = sd(Power, na.rm = TRUE),
               se   = sd / sqrt(N))

fs8 <- ggplot(data = cdata, aes(x = factor(sample_size), y = mean, colour = measure)) +
 geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width = 0.8, 
               position = position_dodge(width = 0.5),
               lwd = 0.25) +
  geom_point(position = position_dodge(width = 0.5), size = 0.5, shape = 16) +
...
```{r, include=FALSE}
ggsave("Figures/FigureS8.pdf", fs8, width = 7, height = 4, useDingbats = FALSE)
```
...
pvalues <- readRDS("Data/EWAS45_pvalues.RDs")
...
```{r, message=FALSE}
# get the finificance order for each cpg
...
panel_B <- panel_B + geom_segment(data = ggplot_build(panel_B)$data[[1]],
...
fs9 <- as.data.frame(ranks[selected_method])[selected_cpg, ] %>%
  melt(value.name = "Significance_rank") %>%
  ...
  ggplot(aes(x = measure, y = Significance_rank)) +
  geom_boxplot(aes(fill = measure)) + 
#  stat_summary(geom = "crossbar", width=0.63, fatten=0, color="white", fun.data = function(x){ return(c(y=median(x), ymin=median(x), ymax=median(x))) }) +
  geom_hline(yintercept = reference_line, linetype = "dashed", color = "black", alpha = 0.5) +
  facet_wrap(~method, nrow = 2, ncol = 2) +
  scale_fill_manual(values = measure_col) +
  scale_x_discrete(limits = selected_measure) +
  xlab("") + ylab("Significance Rank") + 
  theme_bw() +
...
```{r, include=FALSE}
save_plot("Figures/FigureS9.pdf", fs9, base_width = 6.69)
```
...
panel_C <- subset_rank[selected_cpg, ] %>%
  ...
  theme(axis.text.x = element_text(angle = 30, hjust = 0.5, vjust = 0.5),
        legend.position = "none")
panel_C <- panel_C + geom_segment(data = ggplot_build(panel_C)$data[[1]],
                       aes(x = xmin, xend = xmax, y = middle, yend = middle), 
                       color = "white") +
  geom_hline(yintercept = reference_line, linetype = "dashed", color = "black")
panel_C
...
pvalues <- readRDS("Data/EWAS27_pvalues.RDs")
## heatmap
discovery <- as.data.frame(lapply(pvalues, function(x)(apply(x, 2, function(x)sum(x < 0.05)))))
discovery <- as.data.frame(t(discovery))
...
draw(fs10_1)
fs10_1 <- grid.grabExpr(draw(fs10_1))
...
fs10_2 <- ranks[[selected_method]][selected_cpg, ] %>%
  select(selected_measure) %>%
  melt(value.name = "Significance_rank") %>%
  ggplot(aes(x = variable, y = Significance_rank)) +
  geom_boxplot(aes(fill = variable)) + 
...
fs10_2 <- fs10_2 + geom_segment(data = ggplot_build(fs10_2)$data[[1]],
                       aes(x = xmin, xend = xmax, y = middle, yend = middle), 
                       color = "white") +
  geom_hline(yintercept = reference_line, linetype = "dashed", color = "black")
fs10_2
...
selected_method <- c("CAMT", "AdaPT", "IHW", "BL", "FDRreg")
fs10_3 <- subset_rank[selected_cpg, ] %>%
  select(selected_method) %>%
  melt(value.name = "Significance_rank") %>%
  ggplot(aes(x = variable, y = Significance_rank)) +
  geom_boxplot(aes(fill = variable), width = 0.72) +
...
fs10_3 <- fs10_3 + geom_segment(data = ggplot_build(fs10_3)$data[[1]],
                       aes(x = xmin, xend = xmax, y = middle, yend = middle), 
                       color = "white") +
  geom_hline(yintercept = reference_line, linetype = "dashed", color = "black")
...
bottom_row <- plot_grid(fs10_2, fs10_3, labels = c("B", "C"), align = "h", nrow = 1, label_size = 18)
fs10 <- plot_grid(fs10_1, bottom_row, labels = c("A", ""), ncol = 1, label_size = 18)
fs10
```

```{r, include=FALSE}
save_plot("Figures/FigureS10.pdf", plot = fs10, base_width = 6, base_height = 6)
```
...
pvalues <- readRDS("Data/EWAS20.pvalues.RDs")
library(readxl)
...
#### 3.2 Intersetion of method IHW and CAMT
```{r venn plot, fig.width=4, fig.height=3}
# fs12 <- venn.diagram(list(IHW=IHW, CAMT=CAMT), 
#                   filename = NULL,
#                   fill = c("pink", "green"),
#                   cat.pos = c(0, 0),
#                   direct.area = FALSE
#                   )
fs12 <- draw.pairwise.venn(area1 = length(IHW),
                           area2 = length(CAMT),
                           ...
                           cat.pos = c(0, 0),
                           scaled = FALSE)
plot_grid(grobTree(fs12))
```
<br>
<br>

```{r, include=FALSE}
save_plot("Figures/FigureS12.pdf", fs12)
```

#### 3.3 Compare the methylation level of cg18092474 between groups
```{r}
...
```{r, include=FALSE}
save_plot("Figures/FigureS11.pdf", plot = fs11, base_width = 5, base_height = 6)
```
