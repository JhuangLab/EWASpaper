...
for(i in covariate.name.list){
  if(is.element(i, contin.cv)){
    X <- model.matrix(~ splines::ns(cov.df[[i]],6))[,-1]
    starttime <- Sys.time()
    pi0x <- lm_pi0(pValues=cpgResDf.sv$P.value,
                   X=X, smooth.df=3)
    BL_result[[i]] <- p.adjust(cpgResDf.sv$P.value, method = "fdr") * pi0x$pi0
    endtime <- Sys.time()
    BL_time[[i]] <- as.numeric(endtime - starttime, units = "secs")
    print(paste0(i, " done !"))
  }else if(is.element(i, cate.cv)){
    X <- model.matrix(~cov.df[[i]])[, -1]
    starttime <- Sys.time()
    pi0x <- lm_pi0(pValues=cpgResDf.sv$P.value,
                   X=X, smooth.df=3) # X or as.matrix(X) is ok.
    BL_result[[i]] <- p.adjust(cpgResDf.sv$P.value, method = "fdr") * pi0x$pi0
    endtime <- Sys.time()
    BL_time[[i]] <- as.numeric(endtime - starttime, units = "secs")
    print(paste0(i, " done !"))
...
write.csv(BL_result, file = "BL_result.csv", row.names = FALSE, quote = FALSE)
write.csv(BL_time, file = "BL_time.csv", row.names = FALSE, quote = FALSE)

BL_result <- as.data.frame(BL_result)
BL_time <- as.data.frame(BL_time)


## compare probes' number ==================
BL_probes <- list()
for(i in colnames(BL_result)){
  BL_probes[[i]] <- sum(BL_result[[i]] < 0.05)
}
probes <- as.data.frame(BL_probes)
...
write.csv(probes, file = "BL_Discoveries.csv", quote = FALSE)
